...
> **提交前勘误（2026-09-13）：** 本文保留初次审计记录；当前修复状态、验证结果和适用边界以[中文 PR 修复报告](security-audit-2026-09-13-pr.zh-CN.md)及[复扫报告](security-audit-2026-09-13-rescan/report.md)为准。V3 的残余目录预占问题已在归并 PR #1 后采用私有提取方案处理；V13 仅新增可选约束重载，未强制约束生产调用；V9 未提交完整依赖校验。原严重度统计不代表已验证的远程漏洞数量，已安装插件与管理员配置不属于普通远程玩家权限。
