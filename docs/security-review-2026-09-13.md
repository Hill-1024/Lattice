...
Scope: a source review of Lattice at `605b1991212a29265dc6a60ec0c7ba7a986dc48c`, followed by focused remediation. This is not a certification that the whole server is vulnerability-free. This document records the original focused patch from PR #1. It is now combined with the audit-workspace hardening in PR #2; the [consolidated Chinese report](security-audit-2026-09-13-pr.zh-CN.md) describes the final behavior and current validation.
...
No broad public JNI cleanup is included. The original base used the loader checksum helpers in its download path. In the consolidated patch, downloads use trustedDigestFor/verifyTrustedDigest; the legacy checksum helpers now have only test consumers.
